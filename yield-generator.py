...
class YieldGenerator(Maker):
    ...
    def on_tx_unconfirmed(self, cjorder, balance, removed_utxos):
        ...
        source_mixdepth = self.wallet.addr_cache[removed_utxos.values()[0][
            'address']][0]
        debug('source mixdepth = %d' % (source_mixdepth))
        removed_utxos_balance = sum([addrvalue['value']
                                     for addrvalue in removed_utxos.values()])
