...
class CoinJoinTX(object):
	...
	def recv_tx_parts(self, irc, nick, utxo_list, cj_addr, change_addr):
		...
		self.outputs.append({'address': change_addr, 'value':
			calc_total_input_value(self.utxos[nick]) - self.cj_amount
...
def choose_order(cj_amount):
	...
	orders = [(o['counterparty'], o['oid'], o['txfee'] +
	...
	def run(self):
		...
		if 1:
			...
			cjtx = CoinJoinTX(self.irc, cj_amount, [counterparty], [int(oid)],
				[utxo], wallet.get_receive_addr(mixing_depth=1), None, my_tx_fee_contribution, self.finished_cj_callback)
	...
	for command in message[1:].split(command_prefix):
		...
		if chunks[0] == '%quit' or chunks[0] == '%takerquit':
			...
			cjtx = CoinJoinTX(irc, int(amount), [counterparty], [int(oid)],
				...
				wallet.get_change_addr(mixing_depth=0), my_tx_fee_contribution)
