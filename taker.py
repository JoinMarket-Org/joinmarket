...
class CoinJoinTX(object):
	def __init__(self, irc, cj_amount, counterparties, oids, my_utxos, my_cj_addr,
		...
		for c, oid in zip(counterparties, oids):
			irc.privmsg(c, command_prefix + 'fill ' + str(oid) + ' ' + str(cj_amount))
	...
	def recv_tx_parts(self, irc, nick, utxo_list, cj_addr, change_addr):
		...
		print 'fee breakdown for me totalin=%d txfee=%d cjfee_total=%d' % (my_total_in, 
			self.my_txfee, self.cjfee_total)
...
def choose_order(cj_amount):
	...
	return orders[0] #choose the cheapest, later this will be chosen differently
...
class AlgoThread(threading.Thread):
	...
	def run(self):
		...
		if 1:
			...
			utxo, addrvalue = [(k, v) for k, v in self.initial_unspents.iteritems() if v['value'] == 200000000][0]
			counterparty, oid, cj_amount = choose_sweep_order(addrvalue['value'], my_tx_fee)
			self.finished_cj = False
			cjtx = CoinJoinTX(self.irc, cj_amount, [counterparty], [int(oid)],
				[utxo], wallet.get_receive_addr(mixing_depth=1), None,
...
def on_pubmsg(irc, nick, message):
	...
	for command in message[1:].split(command_prefix):
		...
		if chunks[0] == '%quit' or chunks[0] == '%takerquit':
			...
			try:
				oid = int(chunks[1])
				db.execute("DELETE FROM orderbook WHERE counterparty=? AND oid=?;",
					(nick, oid))
			...
			cjtx = CoinJoinTX(irc, int(amount), [counterparty], [int(oid)],
				[my_utxo], wallet.get_receive_addr(mixing_depth=1),
				wallet.get_change_addr(mixing_depth=0), my_tx_fee)
...
def main():
	...
	db = con.cursor()
        db.execute("CREATE TABLE orderbook(counterparty TEXT, oid INTEGER, ordertype TEXT, "
