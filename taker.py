...
class CoinJoinTX(object):
	...
	def send_priv(self, nick, cmd, msg, enc=False):
		if enc:
			self.taker.privmsg(nick, cmd + ' ' + enc_wrapper.encrypt_encode(msg, self.crypto_boxes[nick][1]))
		else:
	...
	def start_encryption(self, nick, maker_pk):
		...
		self.crypto_boxes[nick] = [maker_pk,enc_wrapper.as_init_encryption(self.kp, enc_wrapper.init_pubkey(maker_pk))]
		#send authorisation request
		my_btc_priv = self.taker.wallet.get_key_from_addr(self.taker.wallet.unspent[self.my_utxos[0]]['address'])
