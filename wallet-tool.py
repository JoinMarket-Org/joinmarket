...
def main():
    ...
    # todo: this logic is wrong.  wallet can be undefined.  Not sure..
    wallet = None
    ...
    if method == 'display' or method == 'displayall' or method == 'summary':
        ...
        for m in range(wallet.max_mix_depth):
            ...
            for forchange in [0, 1]:
                ...
                for k in range(wallet.index[m][forchange] + options.gaplimit):
                    ...
                    used = ('used'
                            if k < wallet.index[m][forchange] else ' new')
        ...
        # todo: seed can be undefined.  logic error!!!
        seed = None
        if method == 'generate':
            ...
            words = input('Input 12 word recovery seed: ')
        ...
        walletname = input('Input wallet file name (default: wallet.json): ')
        if len(walletname) == 0:
        ...
        print(('WARNING: Handling of raw ECDSA bitcoin private keys '
               'can lead to non-intuitive '
               'behaviour and loss of funds.\n  Recommended instead is to '
               'use the "sweep" feature of sendpayment.py'))
        ...
        for privkey in privkeys:
            #TODO is there any point in only accepting wif format?
            # check what other wallets do
            ...
            if privkey_format == 'wif':
                #TODO if they actually use an unc privkey, make sure the
                # unc address is used
