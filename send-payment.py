...
def choose_order(db, cj_amount):
    ...
    return orders[
        0][:2]  #choose the cheapest, later this will be chosen differently
...
class PaymentThread(threading.Thread):
    ...
    def run(self):
        ...
        self.taker.cjtx = takermodule.CoinJoinTX(
            self.taker, self.taker.amount, [counterparty], [oid], utxos,
            self.taker.destaddr, self.taker.wallet.get_change_addr(0),
...
def main():
    ...
    print 'starting irc'
    taker = SendPayment(wallet, destaddr, amount, options.txfee,
                        options.waittime)
