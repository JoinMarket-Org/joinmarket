...
class PrivateKey(Base, ECDSA, Schnorr):
    ...
    def serialize(self, compressed=True):
        ...
        res = lib.secp256k1_ec_privkey_export(self.ctx, privser, keylen,
                                              self.private_key, int(compressed))
        assert res == 1
    ...
    def deserialize(self, privkey_ser):
        ...
        res = lib.secp256k1_ec_privkey_import(self.ctx, privkey, privkey_ser,
                                              len(privkey_ser))
        if not res:
            raise Exception("invalid private key")
