...
class ECDSA:  # Use as a mixin; instance.ctx is assumed to exist.
    ...
    def ecdsa_serialize(self, raw_sig):
        ...
        res = lib.secp256k1_ecdsa_signature_serialize_der(self.ctx, output,
                                                          outputlen, raw_sig)
        assert res == 1
    ...
    def ecdsa_deserialize(self, ser_sig):
        ...
        res = lib.secp256k1_ecdsa_signature_parse_der(self.ctx, raw_sig,
                                                      ser_sig, len(ser_sig))
        assert res == 1
    ...
    def ecdsa_serialize_compact(self, raw_sig):
        ...
        res = lib.secp256k1_ecdsa_signature_serialize_compact(self.ctx, output,
                                                              raw_sig)
        assert res == 1
    ...
    def ecdsa_deserialize_compact(self, ser_sig):
        ...
        raw_sig = ffi.new('secp256k1_ecdsa_signature *')
        res = lib.secp256k1_ecdsa_signature_parse_compact(self.ctx, raw_sig,
                                                          ser_sig)
        assert res == 1
    ...
    def ecdsa_signature_normalize(self, raw_sig, check_only=False):
        ...
        result = lib.secp256k1_ecdsa_signature_normalize(self.ctx, sigout,
                                                         raw_sig)
    ...
    def ecdsa_recover(self, msg, recover_sig, raw=False, digest=hashlib.sha256):
        ...
        recovered = lib.secp256k1_ecdsa_recover(self.ctx, pubkey, recover_sig,
                                                msg32)
        if recovered:
            return pubkey
    ...
    def ecdsa_recoverable_convert(self, recover_sig):
        ...
        lib.secp256k1_ecdsa_recoverable_signature_convert(self.ctx, normal_sig,
                                                          recover_sig)
...
class Schnorr:  # Use as a mixin; instance.ctx is assumed to exist.
    ...
    def schnorr_recover(self,
        ...
        recovered = lib.secp256k1_schnorr_recover(self.ctx, pubkey, schnorr_sig,
                                                  msg32)
        if recovered:
            return pubkey
    ...
    def schnorr_partial_combine(self, schnorr_sigs):
        ...
        res = lib.secp256k1_schnorr_partial_combine(self.ctx, sig64, sig64sin,
                                                    len(sig64sin))
        if res <= 0:
...
class PublicKey(Base, ECDSA, Schnorr):
    ...
    def deserialize(self, pubkey_ser):
        ...
        res = lib.secp256k1_ec_pubkey_parse(self.ctx, pubkey, pubkey_ser,
                                            len(pubkey_ser))
        if not res:
    ...
    def combine(self, pubkeys):
        ...
        res = lib.secp256k1_ec_pubkey_combine(self.ctx, outpub, pubkeys,
                                              len(pubkeys))
        if not res:
    ...
    def tweak_mul(self, scalar):
        ...
        res = lib.secp256k1_ec_pubkey_tweak_mul(self.ctx, newpub.public_key,
                                                scalar)
        if not res:
    ...
    def ecdsa_verify(self, msg, raw_sig, raw=False, digest=hashlib.sha256):
        ...
        verified = lib.secp256k1_ecdsa_verify(self.ctx, raw_sig, msg32,
                                              self.public_key)
    ...
    def schnorr_verify(self,
        ...
        verified = lib.secp256k1_schnorr_verify(self.ctx, schnorr_sig, msg32,
                                                self.public_key)
...
class PrivateKey(Base, ECDSA, Schnorr):
    ...
    def _update_public_key(self):
        ...
        self.pubkey = PublicKey(public_key,
                                raw=False,
                                ctx=self.ctx,
                                flags=self.flags)
    ...
    def ecdsa_sign(self, msg, raw=False, digest=hashlib.sha256, randnonce=None):
        ...
        signed = lib.secp256k1_ecdsa_sign(self.ctx, raw_sig, msg32,
                                          self.private_key, nf, ndata)
        assert signed == 1
    ...
    def schnorr_generate_nonce_pair(self,
                                    msg,
                                    raw=False,
                                    digest=hashlib.sha256):
        """
        ...
        valid = lib.secp256k1_schnorr_generate_nonce_pair(
            self.ctx, pubnonce, privnonce, msg32, self.private_key, ffi.NULL,
            ffi.NULL)
        assert valid == 1

        return pubnonce, privnonce

    def schnorr_partial_sign(self,
                             msg,
                             privnonce,
                             pubnonce_others,
                             raw=False,
        ...
        res = lib.secp256k1_schnorr_partial_sign(self.ctx, sig64, msg32,
                                                 self.private_key,
...
def _main_cli(args, out, encoding='utf-8'):
    ...
    def show_public(public_key):
        ...
        out.write(u"Public key: {}\n".format(binascii.hexlify(rawp).decode(
            encoding)))
    ...
    if args.action == 'privkey':
        ...
        out.write(u"{} {}\n".format(
            binascii.hexlify(sig).decode(encoding), recid))
        if args.show_pubkey:
            show_public(priv.pubkey)
