...
def privkey_to_pubkey(privkey):
    ...
    if f in ['bin', 'bin_compressed', 'hex', 'hex_compressed', 'decimal']:
        ...
        return encode_pubkey(
            base10_multiply(G, privkey), f.replace('wif', 'hex'))
...
def random_key():
    ...
    entropy = os.urandom(32) + str(random.randrange(2**256)) + str(int(
        time.time())**7)
...
def encode_sig(v, r, s):
    ...
    return base64.b64encode(vb + '\x00' * (32 - len(rb)) + rb + '\x00' * (
        32 - len(sb)) + sb)
...
def ecdsa_raw_verify(msghash, vrs, pub):
    ...
    u1, u2 = z * w % N, r * w % N
    x, y = base10_add(
        base10_multiply(G, u1), base10_multiply(
...
def ecdsa_raw_recover(msghash, vrs):
    ...
    Qr = base10_add(
        neg_pubkey(base10_multiply(G, z)), base10_multiply(
            (x, y), s))
