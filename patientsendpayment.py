...
class TakerThread(threading.Thread):
	...
	def run(self):
		...
		orders = sendpayment.choose_order(self.tmaker.db, self.tmaker.amount, self.tmaker.makercount)
		print 'chosen orders to fill ' + str(orders)

		utxos = self.taker.wallet.select_utxos(self.tmaker.mixdepth, self.tmaker.amount)
		self.tmaker.cjtx = taker.CoinJoinTX(self.tmaker, self.tmaker.amount,
			orders, utxos, self.tmaker.destaddr,
