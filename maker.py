	...
	def __init__(self, maker, nick, oid, amount, i_utxo_pubkey):
		...
		maker.privmsg(nick, command_prefix + 'io ' + ','.join(self.utxos) + ' ' +
			self.cj_addr + ' ' + self.change_addr + ' ' +
		        btc_pub + ' ' + btc_sig + ' ' + maker.enc_kp.hex_pk())
			...
			try:
				...
				if chunks[0] == 'auth':
					if nick not in self.active_orders:
						self.send_error(nick,"Encryption handshake message received out of order, ignored.")
						continue
					try:
				...
				if chunks[0] == 'fill':
					...
					try:
						...
						self.send_error(nick, str(e))
					self.active_orders[nick] = CoinJoinOrder(self, nick, oid, amount, i_utxo_pubkey)
