...
import enc_wrapper
from common import chunks, errordetect_ordername_list, config, \
...
def random_nick(nick_len=9):
    ...
    print(
        'Generated random nickname: ' + ircnick
...
class IRCMessageChannel(MessageChannel):
    ...
    def check_for_orders(self, nick, _chunks):
        if _chunks[0] in errordetect_ordername_list:
            try:
                counterparty = nick
                oid = _chunks[1]
                ordertype = _chunks[0]
            ...
            try:
                ...
                if _chunks[0] == 'fill':
                    ...
                    if self.on_order_fill:
                        # todo: oid, amount, taker_pk might be unset.  Logic error
                    ...
                    if self.on_seen_auth:
                        # todo: i_utxo_pubkey, btc_sig might be unset
    ...
    def __handle_line(self, line):
        ...
        if _chunks[1] == 'PONG':
            ...
            if target == self.nick:
                self.give_up = True
                raise IOError(get_irc_nick(_chunks[
