...
def _get_nacl():
    ...
    if sys.platform.startswith('win'):
        ...
        for soname_ver in __SONAMES:
            try:
                return ctypes.cdll.LoadLibrary('libsodium-{0}'.format(
                    soname_ver))
            except OSError:
                pass
        try:
        ...
        for soname_ver in __SONAMES:
            try:
                return ctypes.cdll.LoadLibrary('libsodium.so.{0}'.format(
                    soname_ver))
            except OSError:
                pass
        try:
...
def crypto_box_open(ctxt, nonce, pk, sk):
    ...
    ret = nacl.crypto_box_open(msg, pad, ctypes.c_ulonglong(len(pad)), nonce,
                               pk, sk)
    if ret:
...
def crypto_box_afternm(msg, nonce, k):
    ...
    ret = nacl.crypto_box_afternm(ctxt, pad, ctypes.c_ulonglong(len(pad)),
                                  nonce, k)
    if ret:
...
def crypto_box_open_afternm(ctxt, nonce, k):
    ...
    pad = b'\x00' * crypto_box_BOXZEROBYTES + ctxt
    msg = ctypes.create_string_buffer(len(pad))
    ret = nacl.crypto_box_open_afternm(msg, pad, ctypes.c_ulonglong(len(pad)),
                                       nonce, k)
    if ret:
...
def crypto_sign_open(sig, vk):
    ...
    ret = nacl.crypto_sign_open(msg, msglenp, sig, ctypes.c_ulonglong(len(sig)),
                                vk)
    if ret:
...
def crypto_secretbox(msg, nonce, key):
    ...
    ctxt = ctypes.create_string_buffer(len(pad))
    ret = nacl.crypto_secretbox(ctxt, pad, ctypes.c_ulonglong(len(pad)), nonce,
                                key)
    if ret:
...
def crypto_secretbox_open(ctxt, nonce, key):
    ...
    msg = ctypes.create_string_buffer(len(pad))
    ret = nacl.crypto_secretbox_open(msg, pad, ctypes.c_ulonglong(len(pad)),
                                     nonce, key)
    if ret:
...
def crypto_stream_xor(msg, nonce, key):
    ...
    ret = nacl.crypto_stream_xor(stream, msg, ctypes.c_ulonglong(len(msg)),
                                 nonce, key)
    if ret:
        raise ValueError('Failed to init stream')
    return stream.raw
...
def crypto_generichash(msg, key=None):
    ...
    if key:
        ...
        key_len = 0
    nacl.crypto_generichash(hbuf, ctypes.c_ulonglong(len(hbuf)), msg,
