...
def bci_fetchtx(txhash):
    ...
    data = make_request('https://blockchain.info/rawtx/' + txhash +
                        '?format=hex')
    return data
...
def helloblock_fetchtx(txhash, network='btc'):
    ...
    if network == 'testnet':
        ...
        raise Exception('Unsupported network {0} for helloblock_fetchtx'.format(
            network))
...
def firstbits(address):
    if len(address) >= 25:
        ...
        return make_request('https://blockchain.info/q/resolvefirstbits/' +
                            address)


def get_block_at_height(height):
    j = json.loads(make_request("https://blockchain.info/block-height/" + str(
        height) + "?format=json"))
    for b in j['blocks']:
...
def _get_block(inp):
    if len(str(inp)) < 64:
        ...
        return json.loads(make_request('https://blockchain.info/rawblock/' +
                                       inp))
