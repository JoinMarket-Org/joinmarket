...
def deserialize(tx):
    if re.match('^[0-9a-fA-F]*$', tx):
        return json_changebase(
            deserialize(tx.decode('hex')), lambda x: x.encode('hex'))
...
def serialize(txobj):
    ...
    if json_is_base(txobj, 16):
        return serialize(json_changebase(
            txobj, lambda x: x.decode('hex'))).encode('hex')
...
def mk_pubkey_script(
        addr):  # Keep the auxiliary functions around for altcoins' sake
...
def script_to_address(script, vbyte=0):
    ...
    if script[:3] == '\x76\xa9\x14' and script[-2:] == '\x88\xac' and len(
            script) == 25:
...
def deserialize_script(script):
    if re.match('^[0-9a-fA-F]*$', script):
        return json_changebase(
            deserialize_script(script.decode('hex')), lambda x: x.encode('hex'))
    out, pos = [], 0
...
def serialize_script(script):
    if json_is_base(script, 16):
        return serialize_script(json_changebase(
            script, lambda x: x.decode('hex'))).encode('hex')
...
def apply_multisignatures(
        *args):  # tx,i,script,sigs OR tx,i,script,sig1,sig2...,sig[n]
    ...
    if re.match('^[0-9a-fA-F]*$', tx):
        return apply_multisignatures(
            tx.decode('hex'), i, script, sigs).encode('hex')
