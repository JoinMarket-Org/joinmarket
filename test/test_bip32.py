...
def test_bip32_vector(vector):  
    ...
    for i in range(1,len(vector['depths'])):
        ...
        assert currentkey == vector['keys'][i][0], 'failed: child priv key, should be: '+vector['keys'][i][0]
        pub = btc.bip32_privtopub(currentkey)
        print pub
        print vector['keys'][i][1]
        assert pub == vector['keys'][i][1], 'failed: child pub key, should be: '+vector['keys'][i][1]
