...
class Join2PTests(unittest.TestCase):
    ...
    def run_simple_send(self, n, m):
        ...
        dest_address = btc.privkey_to_address(os.urandom(32), from_hex=False,
                                              magicbyte=common.get_p2pk_vbyte())
        try:
            for i in range(m):
...
class JoinNPTests(unittest.TestCase):
    ...
    def run_3party_join(self):
        ...
        dest_address = btc.privkey_to_address(
            ...
            magicbyte=common.get_p2pk_vbyte())
        try:
            sp_proc = local_command([python_cmd,'sendpayment.py','--yes','-N', '3',\
