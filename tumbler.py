...
def generate_tumbler_tx(destaddrs, options):
	...
	for m, txcount in enumerate(txcounts):
		...
		for amount_ratio, wait, makercount in zip(amount_ratios, waits, makercounts):
			tx = {'amount_ratio': amount_ratio, 'wait': round(wait, 2),
				'srcmixdepth': m + options.mixdepthsrc, 'makercount': makercount, 'dest': 'internal'}
			tx_list.append(tx)
...
class TumblerThread(threading.Thread):
	...
	def send_tx(self, tx, sweep, i, l):
		total_value = 0
		all_utxos = self.taker.wallet.get_utxo_list_by_mixdepth()[tx['srcmixdepth']]
		...
		if sweep:
			...
			self.taker.cjtx = takermodule.CoinJoinTX(self.taker.msgchan, self.taker.wallet, self.taker.db,
				cjamount, orders, all_utxos, destaddr, None, self.taker.txfee, self.finishcallback)
		else:
			...
			self.taker.cjtx = takermodule.CoinJoinTX(self.taker.msgchan, self.taker.wallet,
				self.taker.db, amount, orders, utxos, destaddr, changeaddr, self.taker.txfee,
				self.finishcallback)
...
def main():
	...
	parser.add_option('-x', '--maxcjfee', type='float', dest='maxcjfee',
		default=0.02, help='maximum coinjoin fee the tumbler is willing to pay for a single coinjoin. default=0.02 (2%)')
