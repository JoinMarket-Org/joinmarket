...
def generate_tumbler_tx(destaddrs, options):
    ...
    for m, txcount in enumerate(txcounts):
        ...
        for amount_ratio, blockheight_wait, makercount in zip(
            ...
            tx = {'amount_ratio': amount_ratio,
                  'blockheight_wait': blockheight_wait,
    ...
    print('estimated time taken ' + str(block_count * 10) + ' minutes or ' +
          str(block_count / 6.0) + ' hours')
    maker_count = sum([tx['makercount'] for tx in tx_list])
    relorder_fee = 0.001
    print('uses ' + str(maker_count) + ' makers, at ' + str(relorder_fee * 100)
          + '% per maker, estimated total cost ' + str(round(
...
def main():
    ...
    parser.add_option(
        ...
        default=2,
        help=
        'the number of blocks to wait between transactions is randomly chosen '
